...
- **API base URL**: web release builds call the page origin (Caddy serves app + API on one host); dev builds default to `localhost:8000` (`10.0.2.2` on Android emulator). An explicit `--dart-define=SENTINELEDGE_API_BASE_URL` always wins; mobile builds must pass it.
