...
**Model note.** The verification-latency and cost-per-event figures above were
measured against `qwen3.7-plus`, the verification model at the time. The project
now runs `qwen3.8-max`: the 3.7 family is no longer reachable on our account, and
there is no `qwen3.8-plus` tier to move to. These two numbers have **not** been
re-measured on `qwen3.8-max`, so read them as applying to the model named.
...
| **Reason with Qwen Cloud** | `qwen3.8-max` verifies ambiguous events, turns plain-language instructions into monitoring rules, and powers the MCP-connected assistant. |
