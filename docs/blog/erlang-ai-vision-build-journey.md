...
**1. Camera (ESP32-S3).**
A XIAO ESP32-S3 module captures JPEG frames at ~15 FPS, provisions itself by scanning a pairing QR code for Wi-Fi and bridge address, and drives pan/tilt servos.

**2. Edge bridge (laptop).**
The bridge receives frames over LAN or USB and runs the local AI pipeline: YOLO for video detection, YAMNet for audio events, and a local Qwen model (via Ollama) for first-pass triage. Routine activity dies here — it never leaves the building. *(Bandwidth reduction vs. streaming everything to the cloud: [MEASURE: X%].)*
...
**4. App (Flutter).**
Verified alerts stream to the Flutter web and Android app in real time over SSE, with push notifications via FCM for high-severity events.
...
- Camera movement is **clamped to safe mechanical ranges** (pan 0–180°, tilt 60–140°), enforced server-side, so no model output can drive the servos past their limits.
...
The stack held up in testing: **101 backend tests passing**, Flutter analysis clean, and a **zero-hardware demo mode** so judges can experience the full pipeline — real Qwen-VL verification included — without owning an ESP32.

## Three Engineering Challenges That Ate the Hackathon

Three problems consumed far more time than any feature.
...
### Bonus: a design decision that paid off repeatedly
...
- **Measure and publish the numbers**: edge-to-cloud latency, verification latency, bandwidth saved by local triage, and false-positive reduction against a threshold-only baseline.
