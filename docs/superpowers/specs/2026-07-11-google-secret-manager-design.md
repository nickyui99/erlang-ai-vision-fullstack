...
The repository `.env` contains only the local file path. Development reads the
external file directly. During deployment,
...
2. Reads the external credential file in development, or decodes the Base64
   credential injected into production ECI, and parses it in memory.
...
`GOOGLE_SECRET_MANAGER_CREDENTIALS_FILE` is consumed directly by development and
by the deployment script, but is not passed into ECI.
`GOOGLE_SECRET_MANAGER_CREDENTIALS_B64` is generated by that script and is the
only credential source accepted by the production backend.

Local development remains unchanged when Google Secret Manager variables are
absent. When configured, development reads the external service-account file.
If Google configuration is partial, production receives only a file path, or a
configured retrieval fails, startup fails clearly instead of silently falling
back to insecure or stale values.
...
- unreadable development credential file, invalid Base64, or malformed
