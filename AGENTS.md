...
- Backend DB switching (SQLite local vs RDS) is documented in `.env.example`; production secrets (including `DATABASE_URL` and `QWEN_API_KEY`) load from Google Secret Manager via `GOOGLE_SECRET_MANAGER_*`.
